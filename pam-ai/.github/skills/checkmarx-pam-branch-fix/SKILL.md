...
## Fast Path (Seconds)
- Preferred collector: from the `mcp-servers/checkmarx` workspace folder (separate workspace, not inside `pam-frontend`) run `npm run report:branch -- <branch> [--scanId=<SCAN_ID>]`.

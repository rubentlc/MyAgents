...
const tokenUrl = process.env.CHECKMARX_TOKEN_URL ??
...
const buildRequestHeaders = async () => {
    const useRefreshFlow = authScheme.trim().toLowerCase() === "refreshtoken" || looksLikeOfflineToken(apiKey);
    if (useRefreshFlow) {
...
const tryJsonPaths = async (paths) => {
    const errors = [];

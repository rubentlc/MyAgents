---
name: sonarqube-pmt-maintainability-src-fix
description: "Use when you want automatic SonarQube Maintainability remediation for PMT frontend: collect issues impacting MAINTAINABILITY, restrict scope to pmt-frontend/src/*, and apply fixes only inside that path."
argument-hint: "Provide branch name (optional, default: develop). Optionally provide max issues per batch."
user-invocable: true
---

# SonarQube PMT Maintainability (src-only) Remediation

## Purpose
Run a controlled remediation workflow for SonarQube issues impacting `MAINTAINABILITY` in `frontend`, while strictly limiting code changes to `frontend/src/*`.
...
If a Maintainability finding is outside `frontend/src/*`, report it as skipped and do not modify files.
